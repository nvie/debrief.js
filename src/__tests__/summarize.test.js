...
describe('summarize', () => {
    ...
    it('prints annotations with multiple lines', () => {
        ...
        check({ name: annotate(123, 'Must be string') }, ['Value at key "name": Must be string']);
    });
    ...
    it('objects/arrays that are themselves annotated', () => {
        check(
            [{ name: annotate([1, 2, annotate('3', 'Must be number')], 'Must have at most 2 values') }],
            ['Value at keypath 0.name.2: Must be number', 'Value at keypath 0.name: Must have at most 2 values']
        );
        check(
            [annotate({ name: annotate(123, 'Must be number') }, 'Missing key "foo"')],
            ['Value at keypath 0.name: Must be number', 'Value at index 0: Missing key "foo"']
        );
    });
});
